...
export const getRenderableVDOMDiff = (currentRenderableVDOMElement, vdom, parentPointer) => {
    ...
    // added element
    if (!prev) {
      return [{ VDOMPointer: currentRenderableVDOMElement.VDOMPointer, type: diffType.nodeAdded, payload: { node: currentRenderableVDOMElement, parentPointer } }];
    }
  
    // Hopefully this becomes redundant
    // removed element
    if (!currentRenderableVDOMElement) {
      return [{ VDOMPointer: currentRenderableVDOMElement.VDOMPointer, type: diffType.nodeRemoved, payload: {} }];
    }
    ...
    if (
      ...
      return [{ VDOMPointer: currentRenderableVDOMElement.VDOMPointer, type: diffType.nodeReplaced, payload: { newNode: currentRenderableVDOMElement, oldNode: prevElement, parentPointer } }];
    }
    ...
    if (
      ...
      if (prevElement.value !== currElement.value) {
        return [{ VDOMPointer: currentRenderableVDOMElement.VDOMPointer, type: diffType.primitiveNodeUpdate, payload: { newElement: currElement } }];
      }
    ...
    for (var index = 0; index < keys.length; index++) {
      ...
      if (!(key in currElement.props)) {
        changedProps[key] = [propsDiffType.removed, { oldValue: prevElement.props[key] }];
        continue;
      }
  
      if (currElement.props[key] !== prevElement.props[key]) {
        changedProps[key] = [propsDiffType.updated, { newValue: currElement.props[key], oldValue: prevElement.props[key] }];
      }
    }
    ...
    if (Object.keys(changedProps).length > 0) {
      diff.push(
        { VDOMPointer: currentRenderableVDOMElement.VDOMPointer, type: diffType.props, payload: changedProps }
      );
    }
    ...
    for (let index = 0; index < maxIndex; index++) {
      ...
      if (!currChild) {
        diff.push({ VDOMPointer: [...currentRenderableVDOMElement.VDOMPointer, index], type: diffType.nodeRemoved, payload: {} })
        continue;
      }
      const res = getRenderableVDOMDiff(currChild, vdom, currentRenderableVDOMElement.VDOMPointer);
      if (res) {
        diff = [ ...diff, ...res ];
      }
    }
